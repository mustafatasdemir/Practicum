...
public class FullDBLPDataToTextConverter {
	...
	private static void addDblpDataInTextFormat(DBLPElement dblpElement) throws IOException {
		...
		List<BookChapter> inproceedings = (List<BookChapter>)dblpElement.getInproceedingsList();
		if(inproceedings!=null)
		{
			System.out.println("inproceeding : "+inproceedings.size());
			for(BookChapter inproceeding: inproceedings) {
		...
		List<ConferencePaper> proceedings = (List<ConferencePaper>)dblpElement.getProceedingsList();
		if(proceedings!=null)
		{
			System.out.println("proceedings : "+proceedings.size());
			for(ConferencePaper proceeding: proceedings) {
